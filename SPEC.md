...
> ⚠ 중요: `ELS_DN.py`의 KOFIA 직접 API 수집(fetch_kofia_data)은 **동작하지 않음**
> (2026-07-16 실테스트: KOFIA WAF가 requests 기반 호출을 차단, 세션 쿠키로도 불가).
> KOFIA 직접 수집 코드를 이식하려 하지 말 것. 데이터 입수는 아래 §5의 엑셀 임포트 방식만 사용.
...
**2차 로드맵 (지금 구현하지 말 것)**: Playwright 헤드리스 브라우저로 KOFIA
dis.kofia.or.kr 공모정보 페이지 자동화 → exe 의존 제거. (실브라우저 접속은 정상 확인됨)
