...
SETS = [
    ...
    ("현행 레이더 배지", lambda r: r["badge"]),
    ("코어1: 지수·1차<=90·비고점", lambda r: r["t"] == "지수형" and r["b0"] is not None
        and r["b0"] <= 90 and r["peak"] is not None and r["peak"] < 95),
    ("코어2: 코어1+HSCEI제외", lambda r: r["t"] == "지수형" and r["b0"] is not None
        and r["b0"] <= 90 and r["peak"] is not None and r["peak"] < 95 and not r["hscei"]),
    ("위성1: 종목·고점<90·자산<=2·3회+", lambda r: r["t"] == "종목형"
        and r["peak"] is not None and r["peak"] < 90 and r["na"] <= 2 and r["n1y"] >= 3),
    ("위성2: 위성1+1차<=85", lambda r: r["t"] == "종목형"
        and r["peak"] is not None and r["peak"] < 90 and r["na"] <= 2 and r["n1y"] >= 3
        and r["b0"] is not None and r["b0"] <= 85),
]
